...
const TransactionMechanic = () => {
  ...
  // Fetch mechanicId from AsyncStorage
  useEffect(() => {
    const loadMechanicId = async () => {
      try {
        const id = await AsyncStorage.getItem('idUser');
        if (id) {
          setMechanicId(id);
        } else {
          setError('Mechanic ID not found in storage.');
        }
      } catch (err) {
  ...
  // Fetch transactions based on mechanicId
  useEffect(() => {
    const fetchTransactions = async () => {
      if (!mechanicId) return;

      try {
        const response = await axios.get(`http://192.168.18.20:3000/transaksi/mechanic/${mechanicId}`);
  ...
  const handleCompleteTransaction = async (transactionId) => {
    navigation.navigate('PaymentPage', { transactionId });
  };
          ...
          <View style={styles.transactionBox}>
            ...
            <View style={styles.iconContainer}>
              ...
              <TouchableOpacity onPress={() => handleOpenMap(item.latitude, item.longitude)}>
                <Icon name="location-arrow" size={24} color="#00ADB5" style={styles.mapIcon} />
              </TouchableOpacity>

              {/* Complete Transaction Icon */}
              <TouchableOpacity onPress={() => handleCompleteTransaction(item.id_transaksi)}>
                <Icon name="check-circle" size={24} color="#4CAF50" style={styles.completeIcon} />
              </TouchableOpacity>
            </View>
          </View>
        )}
...
export default TransactionMechanic;
